...
module Vexapion
	...
	class Bitbankcc < BaseExchanges
		...
		def initialize(key = nil, secret = nil)
			...
			@public_url = 'https://www.cryptopia.co.nz/api/'
			@private_url = 'https://www.cryptopia.co.nz/api/'
		end
...
#  Create request header & body
		...
		def post(method, params = {})
			...
			md5_string = Digest::MD5.new.digest(body.to_s)
			base64_string = Base64.encode64(md5_string)
			message = "#{@key}POST#{CGI::escape(url).downcase}#{nonce}#{base64_string}".strip
			sign = signature(message)
			header = "amx #{@key}:#{sign}:#{nonce}"
			...
			request['User-Agent'] = 'Mozilla/4.0 (compatible; Cryptopia.co.nz API Ruby client)"
		...
		def signature(message)
			...
			raw = OpenSSL::HMAC.hexdigest(algo, Base64.decode64(@secret), message)
			Base64.encode64(raw).strip
		end
